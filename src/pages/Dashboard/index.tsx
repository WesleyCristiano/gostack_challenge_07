  ...
  useEffect(() => {
    async function loadTransactions(): Promise<void> {
      ...
      transactions.forEach((transaction: Transaction) => {
        transaction.type == 'outcome' ? transaction.formattedValue = `- ${formatValue(transaction.value)}`:
        transaction.formattedValue = formatValue(Number(transaction.value));
        transaction.formattedDate = formatDate(transaction.created_at.toString())
      });
      for (const key in balance) {
        if (balance.hasOwnProperty(key)) {
          balance[key] = formatValue(Number(balance[key]));
